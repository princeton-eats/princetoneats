...
def map_args(hall, date):
    hall = hall.lower()  # normalize input
    if hall in ["r", "roma"]:
        ...
        raise ValueError(f"Unknown dining hall: {hall}")

    if date is None:
        date_url = get_current_date()
    else:
        # Remove leading 0s and join with %2F
        date_parts = [str(int(part)) for part in date.split("/")]
        date_url = "%2F".join(date_parts)
...
def get_details_url(hall, date, meal):
    return (
        f"https://menus.princeton.edu/dining/_Foodpro/online-menu/"
        f"pickMenu.asp?locationNum=01&locationName={hall}&dtdate={date}"
        f"&mealName={meal}&sName=Princeton+University+Campus+Dining"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Princeton dining menu")
    parser.add_argument("--hall", required=True, help="Specify the dining hall")
    parser.add_argument(
        "--meal", required=True, help="Specify the meal (Breakfast, Lunch, Dinner)"
    )
    parser.add_argument(
        "--date", required=False, help="Date in MM/DD/YY format (default is today)"
    )
    args = parser.parse_args()

    try:
        formatted_hall, formatted_date = map_args(args.hall, args.date)
        url = get_details_url(formatted_hall, formatted_date, args.meal)
